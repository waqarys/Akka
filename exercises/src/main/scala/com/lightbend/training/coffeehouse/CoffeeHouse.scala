...
class CoffeeHouse(caffeineLimit: Int) extends Actor with ActorLogging{
  ...
  protected def createGuest(favoriteCoffee: Coffee, guestCaffeineLimit: Int): ActorRef = context.actorOf(Guest.props(waiter, favoriteCoffee, finishCoffeeDuration, guestCaffeineLimit))
